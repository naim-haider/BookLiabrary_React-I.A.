...
import { updateDummyBooks } from "../../redux/booksSlice";

const EditDummyBookPage = () => {
  const { id } = useParams();
  ...
  const dummyBook = useSelector((state) =>
    state.books.dummyBooks.find((book) => book.id === parseInt(id))
  );

  const [title, setTitle] = useState(dummyBook?.title || "");
  const [author, setAuthor] = useState(dummyBook?.author || "");
  const [category, setCategory] = useState(dummyBook?.category || "");
  const [description, setDescription] = useState(dummyBook?.description || "");
  const [rating, setRating] = useState(dummyBook?.rating || "");
  ...
  const handleUpdate = (e) => {
    ...
    if (title && author && category && description && rating && imageUrl) {
      ...
      dispatch(updateDummyBooks(updatedBook));
...
export default EditDummyBookPage;
